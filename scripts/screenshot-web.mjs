...
const smallSize = "1300,400";
const largeSize = "1300,1000";

const smallViews = ["signin", "webexLoading", "noDevices"];
const largeViews = ["control"];

const views = [...smallViews, ...largeViews];
...
try {
  ...
  for (const view of largeViews) {
    await captureView(view, largeSize);
    
  }

  for (const view of smallViews) {
    await captureView(view, smallSize);
  }


  console.log(`Saved ${views.length * themes.length} screenshots to ${outputDir}`);
} finally {
  if (server) server.kill();
}

async function captureView(view, size) {
  for (const theme of themes) {
    const fileName = `screenshot-${view}-${theme}.png`;

    await captureScreenshot({
      filePath: path.join(outputDir, fileName),
      size,
      url: getShotUrl(view, theme),
    });
  }
}
